...
Replacing `IProjectBehavior` changes dialog and project-file handling; it does not replace the project catalog. The replacement should publish metadata and active-selection changes through `IProjectService`. Flourish continues to write mappings to existing local files atomically to the path selected by `InitProjectCatalogFilePath`; transient and stale mappings are excluded. See [Projects](projects.md) for the lifecycle contract.
