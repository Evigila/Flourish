...
`Tonal` 在深浅主题中使用 Fluent Web 的 `colorBrandBackground2` 与 `colorBrandForeground2` 色阶。所有非危险变体继承同一个 `HoverReveal.OverrideColor`，只有 `Danger` 覆盖语义颜色。Elevated 阴影绘制在独立的纯背景模板层上，因此不会影响文字的 ClearType 渲染。
