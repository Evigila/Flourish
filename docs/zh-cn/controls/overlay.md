...
原生 WPF `ToolTip` 与 `FlourishToolTip` 共用包含单个 `Temporary` Overlay 的模板。WPF 从字符串值生成的 Tooltip 也会采用该样式，包括禁用状态下 Button 系列控件显示的提示。WPF `ToolTipService` 仍负责打开、延迟、Popup 定位与关闭，因此嵌套的 Overlay 不设置 `PlacementTarget`。
