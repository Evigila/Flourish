...
使用 `Title` 命名卡片主题，使用 `Text` 提供简短摘要，使用 `Body` 放置细节、状态、控件或组合内容树。如果一张卡片需要多个无关标题或若干相互竞争的主要操作，它很可能应该被拆成多张卡片。
...
1. 每个普通页面章节都是 `Chunk`，并且最多只有一个位于开头的 `ChunkHero`。
2. 每个 `ChunkTitle` 都足够简洁，每个 `ChunkDescription` 都不超过一句简短的目的说明。
3. 详细内容位于 `Card.Title`、`Card.Text`、`Card.Body`，或 `ChunkBody` 内确有必要的纯文字区域。
4. 独立主题或行为已拆分为独立卡片。
5. 非交互表面、整卡操作和视觉展示场景分别使用 `Card`、`CardButton` 与 `IconCard`。
6. 同一变种在不同页面中保持相同语义。
7. 主题覆盖同时适用于亮色与暗色模式，并且颜色不是语义的唯一载体。
8. 内部对齐、Presenter 位置、阅读顺序和键盘顺序彼此一致。
