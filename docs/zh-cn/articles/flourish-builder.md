...
| [依赖注入](configure-services.md) | `ConfigureServices` | 注册应用服务、页面、命令解析器和可替换的 Flourish 服务。 |
...
没有配置分组或固定项时，导航栏会自动列出已注册页面。[导航](navigation.md)说明生成键、页面元数据、缓存行为、显式分组、固定项、校验和运行时字符串导航。
