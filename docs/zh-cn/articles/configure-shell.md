...
例如，[`ConfigureFooter`](configure-footer.md) 可以设置状态文本和 Footer 项，但没有 `UseFooter()` 时 Footer 仍然隐藏。[`ConfigureNavigation`](configure-navigation.md) 可以定义分组和宽度，但没有 `UseNavigation()` 时导航栏仍然隐藏。

## 相关 API

- [`ConfigureTitleBar`](configure-title-bar.md)、[`ConfigureNavigation`](configure-navigation.md)、[`ConfigureFooter`](configure-footer.md) 配置由 Shell 开关启用的区域。
