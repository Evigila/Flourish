...
默认 Profile 服务会把普通登录会话保留在内存中。用户启用“记住登录”后，Flourish 使用 Windows DPAPI 的 `DataProtectionScope.CurrentUser` 保护凭据，并以 `Flourish:Profile:Credential` 键写入应用的标准 User Secrets 文档。登出或禁用“记住登录”会删除该值。
...
这样 Generic Host 与 Flourish 会使用同一 User Secrets 来源。没有 User Secrets provider 时，普通登录仍只保留在内存中，但启用“记住登录”会抛出包含配置指引的 `InvalidOperationException`。User Secrets 本身不加密数据，因此 Flourish 会在写入前加密凭据值。不要把已记住的凭据提交到 `appsettings.json`。
