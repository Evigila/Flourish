...
Flourish Shell 为标题栏、Logo 信息视图、导航栏、动态工具栏、内容区和状态栏提供预定义扩展区域。使用 `ConfigCustomHandler` 可以将应用提供的 WPF 元素或命令放入这些区域。
...
builder
    ...
    .ConfigCustomHandler(custom =>
    {
        custom
            ...
            .InitTitleBarAction("同步", "\uE895", "sync.run")
            .InitFooterCommand(
...
自定义内容不会自动启用所属 Shell 功能。请在 [Shell 配置](shell-configuration.md)中使用 `UseTitleBar()` 启用标题栏区域、使用 `UseNavigation()` 启用导航区域、使用 `UseDynamicToolbar()` 启用工具栏区域，并使用 `UseStatusBar()` 启用页脚区域。`InitProfileContent` 还需要在[标题栏](configure-title-bar.md)中调用 `UseProfile()`。

`FlourishRegion.TitlebarApplicationInfo` 区域会渲染为 Logo 信息视图的 Body。添加此内容前应通过 `UseLogo()` 配置 Logo。Body 完全由应用定义，可以显示动态详情，但不会参与项目新建、保存、激活、删除或关闭处理。这些 Shell 入口由 `IProjectBehavior` 协调；参见[项目](projects.md)。
