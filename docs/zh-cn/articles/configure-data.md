---
title: 应用数据
description: 配置 Flourish 本地化、共享 Generic Host 配置与相邻的项目目录。
---
...
`IProjectService` 将有序项目元数据与活动项目 ID 存储在 `projects.json`。该文件与 `IAppSettingsStore.FilePath` 位于同一目录，通常和基础 `appsettings.json` 相邻，但它不是 Host 配置源，也不参与配置优先级。
