...
2. 使用 [`ConfigureDynamicToolbar`](configure-dynamic-toolbar.md) 注册页面对应的工具栏项。
...
在 [`ConfigureServices`](configure-services.md) 中注册一个或多个 `ICommandParser` 实现。
...
识别并处理成功时返回 `true`。返回 `false` 表示当前解析器不处理该命令，允许其他解析器继续尝试。[`ConfigureCustomHandler`](configure-custom-handler.md) 可以让标题栏和 Footer 命令复用同一组 command key。
