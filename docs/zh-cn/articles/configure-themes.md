...
Flourish 使用 Fluent 颜色系统的精简子集。中性色角色负责文字、表面、交互背景、
禁用状态与描边；主要色、辅助色和强调色构成应用配色；危险与警告角色用于传达状态。

控件使用这些语义角色，不再分别维护组件专用颜色。标准表面共用同一套悬停和按下层级，
其中包括卡片形态的选择控件；选中内容使用统一的选择角色，展示型覆盖层也共用一组遮罩
与前景色。只有品牌或状态变体在语义、对比度确实不同时保留独立的交互颜色。

编写自定义模板时，应引用用途最接近的 Flourish 语义资源，不要直接指定颜色或创建组件
专用悬停笔刷。这样可以让模板在亮色、暗色和运行时自定义主题之间保持一致。
