...
公共控件库包含 17 个控件。除下表明确列出其他基类外，每个控件都继承对应的 WPF 控件，并保留该基类的属性、事件、命令、数据绑定、验证和自动化行为。

| Flourish 控件 | 基类 | Flourish 特有契约 |
| --- | --- | --- |
| `FlourishButton` | `Button` | `Appearance` 与 `Variant` 分别选择语义颜色和布局角色。 |
...
`FlourishButton.Appearance` 描述操作语义，默认值为 `Standard`：
...
`FlourishButton.Variant` 描述操作的承载位置，默认值为 `Standard`：

- `Icon` 是紧凑的方形图标操作。
- `Toolbar` 是紧凑的工具栏命令。
- `Action` 是对话框或表单操作。
- `WindowCaption` 使用原生标题栏按钮几何。
- `StatusIcon` 是小型状态栏图标操作。
- `MessageBox` 是 Flourish 消息框中的操作。

`Appearance` 与 `Variant` 相互独立。例如，主要对话框操作应同时设置两者：
...
`FlourishCard.Appearance` 默认值为 `Standard`，其他取值为 `Subtle`、`Accent`、`Elevated` 和 `Hero`。`Elevated` 提供层次效果，`Hero` 使用渐变背景和层次效果显示引导内容。卡片不是输入控件；需要让整个表面可交互时，请使用 `FlourishButton Appearance="Card"`。
...
```xml
<flourish:FlourishButton Content="刷新">
...
`FlourishButton`、`FlourishComboBoxItem` 和 `FlourishListBoxItem` 使用公共 `HoverReveal` 附加行为。通过[动效](configure-motion.md)进行应用级配置，并遵循操作系统的减少动态效果偏好：
...
接入该行为的自定义模板应提供名为 `HoverChrome` 和 `HoverRevealScale` 的元素，设置 `flourish:HoverReveal.IsParticipant="True"`，并将 `flourish:HoverReveal.IsMotionEnabled` 绑定到 `{DynamicResource FlourishHoverRevealEnabled}`。`IsEnabled` 和 `AnimationDuration` 会沿视觉树继承，`IsMotionEnabled` 与 `IsParticipant` 不继承。缺少任一命名元素时，该行为不生效。
